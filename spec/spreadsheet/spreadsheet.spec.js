...
describe('Base26', function() {
  eval(require('fs').readFileSync(__dirname + '/../../out/methane.js')+'');
  
  it('should 10 => J', function() {
    expect(Base26.toBase26(10)).toEqual('J');
  });

  it('should J => 10', function() {
    expect(Base26.toInt('J')).toEqual(10);
  });

  it('should A1 => 1, 1', function() {
    expect(Base26.toRC('A1')).toEqual({r:1, c:1});
  });

  it('should J15 => 15, 10', function() {
    expect(Base26.toRC('J15')).toEqual({r:15, c:10});
  });

  it('should A1:B2 => 1, 1 / 2, 2', function() {
    expect(Base26.range('A1:B2')).toEqual({ 
      'start' : { r:1, c:1 },
      'end'   : { r:2, c:2 }
    });
  });

  it('should A:J => false, 1 / false, 10', function() {
    expect(Base26.range('A:J')).toEqual({ 
      'start' : { r:false, c:1 },
      'end'   : { r:false, c:10 }
    });
  });
